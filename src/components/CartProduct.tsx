...
const CartProduct = ({item}: cartProductProps) => {
  ...
  return (
    <div className='bg-gray-100 rounded-lg flex items-center gap-4  text-amazon_blue'>
    ...
    <div className='flex items-center px-2 gap-4'>
       <div className='flex flex-col gap-1'> 
        ...
        <p className='text-sm text-gray-600'> {item.description.substring(0,190)}</p>
        <p className='text-sm text-gray-600'> Unit Price{" "}<span className='font-semibold text-amazon_blue'>
            <ChangedPrice amount={item.price}/></span></p>
    
       <div className='flex items-center gap-6'>
      <div  className='flex items-center mt-1 lg:mb-0 sm:mb-3  justify-between border border-gray-300 px-4 py-1 rounded-full w-28 shadow-lg shadow-gray-300'>
            <span  onClick={()=>
  ...
             ))} className='flex items-center text-sm font-medium text-gray-400 hover:text-red-600 cursor-pointer duration-300'>
       ...
       <div className='font-semibold  text-amazon_blue text-lg'>
        <ChangedPrice amount={item.price * item.quantity} />
       </div>
